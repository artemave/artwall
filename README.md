...
The trash survives Ctrl-C, and reboots. The only irreversible act in artwall is
the **Delete n paintings forever** button on the trash page.

```bash
./bin/artwall --stars    # gallery, with a Trash link once something's in it
```
...
├── stars.html        the gallery (regenerated by --stars)
├── images/
...
That archived `stars.html` is rewritten on every `--stars` (and on every unstar,
restore or delete). It's the same gallery *without* the buttons or the trash
link, since there's no server behind it once the command exits.
